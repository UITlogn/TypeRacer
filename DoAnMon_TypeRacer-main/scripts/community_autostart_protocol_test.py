...
def wait_for_type(sock: socket.socket, expected: int, timeout: float = 8.0) -> dict | None:
    ...
    while time.time() < deadline:
        ...
        if msg_type == DISCONNECT:
            raise RuntimeError(f"server disconnected: {payload}")
    raise TimeoutError(f"timeout waiting for {expected}")


def main() -> int:
    parser = argparse.ArgumentParser(description="Wait for community Quick Play auto-start with heartbeat")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=5000)
    parser.add_argument("--timeout", type=float, default=230.0)
    args = parser.parse_args()

    username = f"autoquick_{int(time.time())}_{random.randint(100, 999)}"
    ...
    with socket.create_connection((args.host, args.port), timeout=10) as sock:
        ...
        if join.get("race_in_progress"):
            race = join.get("current_race") or {}
            elapsed = race.get("race_elapsed_seconds")
            print(f"QUICK already racing; join-in-progress ok, elapsed={elapsed}s")
            return 0

        room = join.get("room") or {}
        countdown = room.get("seconds_until_next_start")
        print(f"Waiting for QUICK auto-start; countdown={countdown}s")
        ...
        while time.time() < deadline:
            ...
            if now >= next_heartbeat:
                send_message(sock, HEARTBEAT_PING)
                next_heartbeat = now + 20.0

            sock.settimeout(min(1.0, max(0.2, deadline - now)))
            try:
                msg_type, payload = recv_message(sock)
            ...
            if msg_type == RACE_START:
                print(
                    "QUICK auto-start ok: "
                    f"passage_len={len((payload or {}).get('passage_text', ''))}, "
                    f"elapsed={(payload or {}).get('race_elapsed_seconds')}"
                )
                return 0
            if msg_type == ERROR:
                raise RuntimeError(f"server error: {payload}")
        ...
        raise TimeoutError(f"timeout waiting for QUICK RACE_START after {args.timeout}s")
...
if __name__ == "__main__":
    try:
        ...
        print(f"QUICK AUTO-START TEST FAILED: {exc}", file=sys.stderr)
        raise SystemExit(1)
